CREATE VIEW AceitacoesAceitasVsRejeitadas
AS SELECT A.artista AS artista, R.cliente AS cliente, A.aceitacoesAceitas, R.aceitacoesRejeitadas
	FROM(SELECT artista, cliente, COUNT(*) AS aceitacoesAceitas
			FROM Aceitacao
			WHERE status > 4
			GROUP BY artista, cliente) A
		JOIN
		(SELECT artista, cliente, COUNT(*) AS aceitacoesRejeitadas
			FROM Aceitacao
			WHERE status == 1
			GROUP BY artista, cliente) R
...
SELECT MA.artista AS artista, MA.clienteMaisAceitas, MR.clienteMaisRejeitadas
	FROM (SELECT artista, COUNT(*) AS clienteMaisAceitas
